...
import { getOneTrackerAction, updateTrackerAction } from "@/app/server/actions";
...
function EditPackage() {
  ...
  const handleForm2Submit = async function () {
    setLoading(true);
    const result = await updateTrackerAction(id, form1);
    if (result.success) {
      return router.push("/track/" + result.id);
    }
          ...
          {position == 0 ? (
            loading ? (
              ...
              <div className="text-center text-primary">
                <p>{error}</p>
                <Link href="/admin/dashboard" className="underline text-secondary hover:text-secondary/80">
                  Go back
                </Link>
              </div>
            ) : (
