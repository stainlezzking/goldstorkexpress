...
import blog_3 from "@/app/assets/blog-3.png";
...
export default function BlogPreview({ preview, index, id }) {
  ...
  return (
    <div className="max-md:flex-wrap gap-y-10 max-md:w-[500px] max-md:mx-auto flex gap-x-5 pt-[29px] border-t border-[#D6D6D6] max-w-full pb-5 ">
      <Image src={selector[index]} alt="" className="aspect-video max-md:flex-grow self-start w-2/5 object-cover" />
      ...
      <div className="basis-full md:border-s border-[#D6D6D6] min-h-[200px] md:px-5 space-y-3">
        <Link href={"/blogs/" + preview._id} className="text-xl md:text-2xl hover:text-primary">
          {preview.title}
        </Link>
        <p className="text-paragraph">{preview.summary}</p>
        <Button variation="default" onClick={() => router.push("/blogs/" + id)}>
          Read More
        </Button>
      </div>
    </div>
